...
{   
    ...
    public function getAllComments($articleId){
        ...
        try {
            Log::info("Attempting to read all comments on article $articleId");
            // Читаем кешированные комменты из Redis
            $cachedComments = $this->getRedisComments($articleId);
            $result = array_merge($result, $cachedComments);
            
            // Читаем сохраненные комменты из БД
            $stmt = $this->pdo->prepare("
                SELECT c.content, c.created_at, u.full_name, u.avatar_url 
                FROM comments c 
            ...
            // Добавляем комменты из БД
            foreach ($dbComments as $comment) {
                $result[] = [
                    'user' => [
                        'full_name' => $comment['full_name'] ?? 'Unknown',
            ...
            Log::error("Database error in getAllComments: {$e->getMessage()}");
            return [
                'success' => false,
                'message' => 'Server error'
            ];
    ...
    /**
     * Приватный метод для получения комментариев из Redis очереди
     * Не выбрасывает исключения, возвращает пустой массив при ошибке
     */
    private function getRedisComments($articleId) {
        $result = [];
        ...
        try {
            ...
            for ($i = 0; $i < $queueLength; $i++) {
                $item = json_decode($this->redisClient->lIndex($queueKey, $i), true);
                if ($item && $item['article_id'] == $articleId) {
                    // Получаем данные пользователя
                    $user = $this->getUserDataForComment($item['user_id']);
            ...
            Log::error("Redis error in getRedisComments: {$e->getMessage()}");
            return []; // Возвращаем пустой массив при ошибке Redis
        } catch (\Exception $e) {
            Log::error("Error in getRedisComments: {$e->getMessage()}");
            return [];
        }
    ...
    /**
     * Приватный метод для получения данных пользователя для комментария
     * Не выбрасывает исключения
     */
    private function getUserDataForComment($userId) {
        if (!isset($this->redisClient)) {
            return $this->getUserDataFromDb($userId);
        }
        
        try {
            $userData = $this->redisClient->get("user:{$userId}");
            if ($userData) {
                return json_decode($userData, true);
            }
        } catch (\RedisException $e) {
            Log::error("Redis read error for user {$userId}: {$e->getMessage()}");
        }
        
        // Fallback: читаем из БД
        $user = $this->getUserDataFromDb($userId);
        
        // Пытаемся сохранить в Redis для последующих запросов
        if ($user && isset($this->redisClient)) {
            try {
                $this->redisClient->set("user:{$userId}", json_encode($user, JSON_UNESCAPED_UNICODE));
            } catch (\RedisException $e) {
                // Игнорируем ошибку кеширования
                Log::error("Redis write error for user {$userId}: {$e->getMessage()}");
            }
        }
    ...
    /**
     * Приватный метод для получения данных пользователя из БД
     */
    private function getUserDataFromDb($userId) {
        try {
            $stmt = $this->pdo->prepare("SELECT full_name, avatar_url FROM users WHERE id = ?");
            $stmt->execute([$userId]);
            return $stmt->fetch(\PDO::FETCH_ASSOC);
        } catch (\PDOException $e) {
            Log::error("Database error getting user {$userId}: {$e->getMessage()}");
            return null;
        }
    }
}
