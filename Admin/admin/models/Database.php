...
class Database {
    ...
    private function createConnection() {
        ...
        while ($retryCount <= self::MAX_RETRIES){
            try {
                ...
                error_log("Database connection attempt " . ($retryCount + 1) . " failed: " . $errorMessage);
                
                if (strpos($errorMessage, 'max_user_connections') !== false) {
                    $retryCount++;
                    if ($retryCount <= self::MAX_RETRIES) {
                        error_log("Waiting " . self::RECONNECT_DELAY . " seconds before retry...");
    ...
    public static function closeSharedConnection() {
        if (self::$sharedPdo !== null) {
            self::$sharedPdo = null;
            error_log("Shared database connection closed");
        }
    }
    ...
    private function handleCriticalError($errorMessage) {
        error_log("CRITICAL DB ERROR: " . $errorMessage);
        ...
        if (file_exists(__DIR__ . '/../views/errors/database_error.html')) {
            include __DIR__ . '/../views/errors/database_error.html';
        } else {
            logMessage('Service Temporarily Unavailable');
            logMessage('The database is currently experiencing high load. Please try again in a few moments.');
        }
        exit;
    }
